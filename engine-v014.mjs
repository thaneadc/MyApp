...
export function act(original,a,rng=Math.random){const s=normalizeSharedPools(structuredClone(original)),p=current(s),e=s.exp;must(s.version==='0.15'&&s.status==='playing','Start a v0.15 voyage');
 ...
 else if(a.type==='refresh'){must(loc==='black'&&!s.blackRefreshed,'Refresh unavailable');must(Array.isArray(a.ids)&&a.ids.length>=1&&a.ids.length<=3,'Select 1–3 market cards');must(p.gold>=a.ids.length,'Not enough Gold to refresh those cards');for(const id of a.ids){const index=s.market.indexOf(id);must(index>=0,'Select only available market copies');s.market.splice(index,1)}p.gold-=a.ids.length;s.crewDeck=shuffle([...s.crewDeck,...a.ids],rng);while(s.market.length<3&&s.crewDeck.length)s.market.push(s.crewDeck.shift());s.blackRefreshed=true;return s}
