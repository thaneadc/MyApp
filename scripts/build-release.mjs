...
for(let i=0;i<manifest.length;i+=6){
 await Promise.all(manifest.slice(i,i+6).map(async item=>{
  if(!/^(assets\/[\w.-]+|index\.html|menu\.js|polish\.css|cards-v014\.js|engine-v014\.mjs|voyage-v014\.js|voyage-v(?:08|014)\.css|voyage-flair\.js|voyage-feedback\.css)$/.test(item.path))throw new Error('Unexpected asset path');
