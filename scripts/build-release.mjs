...
const required=['index.html','menu.js','polish.css','cards-v014.js','engine-v014.mjs','voyage-v014.js','voyage-v08.css','voyage-v014.css','voyage-flair.js','voyage-feedback.css','voyage-table.css','assets/silver-pirate-hat.png','assets/pirate-theme.ogg','assets/pirate-theme.mp3',...['board','crew','treasure','zone1','zone2','zone3','final'].map(name=>`assets/v08-${name}.png`),...[1,2,3,4].map(n=>`assets/guide-${n}.jpeg`)];
...
for(let i=0;i<manifest.length;i+=6){
 await Promise.all(manifest.slice(i,i+6).map(async item=>{
  if(!/^(assets\/[\w.-]+|index\.html|menu\.js|polish\.css|cards-v014\.js|engine-v014\.mjs|voyage-v014\.js|voyage-v(?:08|014)\.css|voyage-flair\.js|voyage-feedback\.css|voyage-table\.css)$/.test(item.path))throw new Error('Unexpected asset path');
