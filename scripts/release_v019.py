...
# Work from the already-built v0.19 source. Locked zones show only card backs.
...
if old_locked in s:
    ...
    raise SystemExit('locked-zone render marker not found')

# While Dock is active, only reachable Missions are interactive and highlighted.
...
# Dock no longer opens a blocking mission-list modal; return to the board instead.
...
# Styling for card backs and Dock selection mode.
...
.zoneLocked .zoneCards{gap:10px;align-items:center;justify-content:center}
...
checks = {
    'voyage-v014.js': ['missionCardBack', 'dockSelectable', 'dockUnavailable', 'Choose a glowing Mission card on the board.'],
